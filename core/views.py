...
def register(request,competition_name):
    if request.method == "POST":
        prev_performance = request.POST.get('performance')
...
@login_required(login_url='Vogue-Home')
def data(request,comp):
    competition = Competition.objects.get(competition_name=comp)
    Teams=Team.objects.filter(competition=competition)
    return render(request, 'core/data.html',{"teams": Teams});
